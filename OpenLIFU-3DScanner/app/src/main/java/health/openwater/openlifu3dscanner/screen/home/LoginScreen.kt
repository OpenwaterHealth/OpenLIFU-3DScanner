...
fun LoginScreen(
    ...
    onResetPassword: suspend (String) -> Boolean
) {
    ...
    if (showResetPasswordDialog) {
        ResetPasswordDialog(
            ...
            onResetPassword = { resetEmail ->
                scope.launch {
                    val success = onResetPassword(resetEmail)
                ...
                keyboardActions = KeyboardActions(
                    onDone = {
                        ...
                        if (email.isNotEmpty() && password.isNotEmpty()) {
                            scope.launch {
                                isLoading = true
                                val response = onSignIn(email, password)
                                isLoading = false
            ...
            Button(
                onClick = {
                    scope.launch {
                        ...
                        when (response) {
                            AuthService.AuthResponse.INVALID_CREDENTIALS -> {
                                errorMessage = context.getString(R.string.invalid_email_or_password)
                            }
